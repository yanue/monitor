...
check process consul
    ...
    if failed port 8400 use type tcp then restart
END
)
    ...
    do
        ...
        if ! [ -f "/etc/systemd/system/$serviceName.service" ]; then
            ...
            if [[ ${nameWithPid[$runName]} != "" ]]; then
                ...
                newRunName=$(ps -up $pid | grep $runName | awk '{print $11}' | awk '{n=split($1,A,"/"); print A[n]}' | sed -e "s/-service//")
                ...
                if ! [ -f "/etc/systemd/system/$serviceName.service" ]; then
                    echo -e "\033[31m> no found runName:$runName, service:$serviceName, try found by remove blemobi prefix \033[0m"

                    serviceName=$monitName
                    # check systemd file again
                    if ! [ -f "/etc/systemd/system/$serviceName.service" ]; then
